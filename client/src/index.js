...
function Auth(ComposedClass, reload, adminRoute = null) {
  function AuthenticationCheck(props) {
    ...
    return <ComposedClass {...props} redux={redux} />;
  }
          ...
          <div className="register-form-app">
            ...
            >
              <Form.Item
                required
                className={
                  errors.name && touched.name ? "form-item error" : "form-item"
                }
                label="Name"
              >
                <Input
  ...
  return (
    <div className="landing-page">
      <div className="landing-page-control">
        <button onClick={() => onShowOnlyMine()}>
          {ShowOnlyMine ? "Show all" : "Show only mine"}
        </button>
        <button onClick={() => onSorting()}>
          {SortByName === -1
            ? "Don`t sort by title"
            : SortByName === 0
            ? "Sort title by asc order"
            : "Sort  title by desc order"}
        </button>
        <input
          placeholder="star"
          value={ShowStar}
      ...
      <div className="mapofFilms">
        {ShowFilms.map((film, index) => (
          <div key={index} className="filmCard">
            ...
            <ul className="filmCardStars">
              {film.stars.map((star, starindex) => (
        ...
        {!ShowFilms.length && (
          <div className="no_movies_found">
            <div className="no_movies_found_title">no movies found</div>
            <Link to="/upload">add movies</Link>
          </div>
        )}
        
      </div>
      {
          PostSize > ShowFilms.length &&
          <div className="get_more_films"><button onClick={() => onLoadMore()}>get more films</button></div>
                  }
    </div>
  );
}
function FileUpload(props) {
        ...
        if (x % 4 === 0) {
          if (arrwithstrs[x].indexOf("Title:") === 0) {
            if (arrwithstrs[x].trim().length > "Title:".length)
              ObjectForSend.title = arrwithstrs[x]
                .substr("Title:".length, arrwithstrs[x].length)
                .trim();
            else error = "Title is empty somewhere";
          } else error = "Title are not set somewhere";
          x++;
        }
        if (x % 4 === 1) {
            ...
            if (arrwithstrs[x].length > "Release Year:".length) {
              if (
                Number.isInteger(
                  Number(
                    arrwithstrs[x]
                      .substr("Release Year:".length, arrwithstrs[x].length)
                      .trim()
                  )
                ) &&
                arrwithstrs[x]
                ...
                if (
                  ...
                  ObjectForSend.year = arrwithstrs[x]
                    .substr("Release Year:".length, arrwithstrs[x].length)
                    .trim();
                else error = "Wrong year";
              else error = "Release Year is not a number somewhere";
            } else error = "Release Year are empty somewhere";
          } else error = "Release Year are not set somewhere";
          x++;
        }
        if (x % 4 === 2) {
          if (arrwithstrs[x].indexOf("Format:") === 0) {
            let format = arrwithstrs[x]
              .substr("Format:".length, arrwithstrs[x].length)
              .trim();
            if (format === "DVD" || format === "VHS" || format === "Blu-Ray")
              ObjectForSend.format = arrwithstrs[x]
                .substr("Format:".length, arrwithstrs[x].length)
                .trim();
            else error = "Format is not right somewhere";
          } else error = "Format are not set somewhere";
          x++;
        }
        if (x % 4 === 3) {
          if (arrwithstrs[x].indexOf("Stars:") === 0) {
            var stararr = arrwithstrs[x]
              .trim()
              .substr("Stars:".length, arrwithstrs[x].length)
              .split(",");
            ...
            if (
              arrwithstrs[x]
                .trim()
                .substr("Stars:", arrwithstrs[x].length)
                .trim().length > 0
            )
              ObjectForSend.stars = stararr;
            else error = "Stars are not set somewhere";
          } else error = "Stars are not set somewhere";
  ...
  const uploadFile = () => {
    axios.post(`${FILM_SERVER}/upload/films`, MainObj).then((res) => {
  ...
  return (
    <>
      <div className="image-uploader">
        {SuccesFile && <div className="AllOkUpload">{SuccesFile}</div>}
        {ErrorFile && <div className="ErrorUpload">{ErrorFile}</div>}
...
function UploadFilmPage(props) {
  ...
  const onSubmit = (event) => {
    ...
    axios.post(`${FILM_SERVER}/upload/film`, variables).then((response) => {
  ...
  const addStarToArr = () => {
    if (Stars.indexOf(StarInput.trim()) !== -1) alert("This star exist");
      ...
      <Form className="upload-form">
        ...
        <div className="starsMap">
          {Stars.map((star, index) => (
            <div className="starRow" key={index + Math.random()}>
              <div>{star}</div>
              <button
                onClick={() => {
                  setStars(deleteStar(index));
                }}
              >
                X
              </button>
...
function FilmPage(props) {
  ...
  const updatePage = () => {
    ...
    if (redux?.userData?._id)
      axios.post(`${FILM_SERVER}/getfilm`, variables).then((response) => {
  ...
  const onSubmit = () => {
    ...
    axios
      .post(`${FILM_SERVER}/update/film/${filmId}`, variables)
      .then((response) => {
        alert(response.data.message);
        if (response?.data?.data) setFilm(response.data.data);
      });
  };
  const [Important, setImportant] = useState(false);
  ...
  const onDelete = () => {
    if (window.confirm("Are you sure?"))
      axios.delete(`${FILM_SERVER}/delete/film/${filmId}`).then((response) => {
        alert(response.data.message);
  ...
  const getUserFilms = () => {
    if (redux?.userData?._id) {
      axios.post(`${FILM_SERVER}/my/films`).then((res) => {
        ...
        if (FilmsInput) films.filter((film) => film.title === FilmsInput);
        setFilms(films);
      });
    }
  };
      ...
      {(props) => {
        ...
        return (
          <div className="settings-app">
            ...
            <div className="settings_user-name">
              {redux?.userData?.name} | {redux?.userData?.email}
            </div>
            ...
            <div className="deleteBtns">
              <input
                placeholder="Find film for delete"
                value={FilmsInput}
                onChange={(e) => setFilmsInput(e.target.value)}
              />
              {Films.map((item, index) => (
                <div className="delete-row" key={index}>
                  <div>{item.title}</div>
                  <button onClick={() => rmrfFilm(item._id)}>X</button>
                </div>
              ))}
            </div>
          </div>
